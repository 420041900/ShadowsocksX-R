...
func SyncPac() {
    ...
    if !fileMgr.fileExists(atPath: PACByPassChinaFilePath) {needGenerate = true}
    ...
    do {
        ...
        if let data = Data(base64Encoded: gfwlist, options: .ignoreUnknownCharacters) {
            ...
            do {
                ...
                let src = Bundle.main.path(forResource: "bypasschina", ofType: "txt")
                print(src)
                let pacFile = try? Data(contentsOf: URL(fileURLWithPath: src!))
//                let pacFile = NSData(contentsOfURL: NSURL(string: src!)!);
//                print(pacFile)
                var pacStr = String(data: pacFile!,encoding: String.Encoding.utf8)!
                pacStr = pacStr.replacingOccurrences(of: "__PROXY__", with: "SOCKS5 127.0.0.1:\(socks5Port);SOCKS 127.0.0.1:\(socks5Port)")
                try pacStr.data(using: String.Encoding.utf8)?.write(to: URL(fileURLWithPath: PACByPassChinaFilePath), options: .atomic)
//
                return true
            } catch {
